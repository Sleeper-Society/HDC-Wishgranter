...
#### Loading the mod
* `load` (required): A function for inserting new data into the game. This is where calls to `Game.AddCard`,`Game.AddEncounter`,`Game.AddStartingDeck`, `Game.AddDongle`, and `Game.AddKeyword` should occur. Returns either void or a `load_sequence_element` array.
...
### On Loading
```ts
type load_sequence_function = (hyperspace_path : string, mods_path : string, game : Game) => load_sequence_returns
