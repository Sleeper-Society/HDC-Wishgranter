declare namespace gdjs {
    namespace evtTools {
        namespace network {
            ...
            const sendAwaitableAsyncRequest: (url: string, body: string, method: string, contentType: string, responseVar: gdjs.Variable, errorVar: gdjs.Variable) => PromiseTask<void>;
            const retryIfFailed: <T>({ times, delayInMs }: {
                times: number;
                delayInMs?: number;
            }, functionCalled: () => Promise<T>) => Promise<T>;
            /**
             * @deprecated
             */
