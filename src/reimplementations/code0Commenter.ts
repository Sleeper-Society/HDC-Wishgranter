...
export function commentCode0(
    ...
    const scene_variable_replacer = (
        ...
        const variable_name =
            hyperspace_data.layouts[0].variables[
                Number.parseInt(capture_groups[0])
            ].name;
        return ["", ","].includes(capture_groups[1]) ?
                `${capture_groups[0]} /* ${variable_name} */)${capture_groups[1]}`
    ...
    const object_variable_replacer = (
        ...
        return ["", ","].includes(capture_groups[2]) ?
                `${capture_groups[1]} /* ${variable_name} */)${capture_groups[2]}`
