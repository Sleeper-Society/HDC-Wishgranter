...
export class LoadingBarElement extends HTMLElement {
    ...
    >(
        ...
        const new_sub_bars: Record<number, HTMLElement> =
            load_sequence.length ?
            ...
            :   new Proxy(
                    ...
                    {
                        get() {
                            bar.classList.add("unknown-length");
                            return bar;
                        },
                    },
                );
        (
            load_sequence[
                Symbol.iterator
            ]() as IteratorObject<LoadSequenceElement>
        ...
        bar.classList.remove("unknown-length");
...
> {
    ...
    function:
        ...
        | ((...args: CallParamaters) => Iterable<
              LoadSequenceElement<CallParamaters>
          > & {
              length?: number;
          })
        | ((...args: CallParamaters) => Promise<
              Iterable<LoadSequenceElement<CallParamaters>> & {
                  length?: number;
              }
          >);
}
